...
class User extends DB
{
    ...
    SELECT COUNT(*) as co, date(created_at) as date, 1 as status from tasks where  datediff(CURRENT_TIMESTAMP, created_at) < 30 and responsible = '$this->id' GROUP BY date
UNION
SELECT   COUNT(*) as co, date(updated_at) as date, 2 as status from tasks where  datediff(CURRENT_TIMESTAMP, updated_at) < 30 and responsible = '$this->id' GROUP BY date
UNION
SELECT  COUNT(*) as co, date(finished_at) as date, 3 as status from tasks where datediff(CURRENT_TIMESTAMP, finished_at) < 30 and responsible = '$this->id' GROUP BY date
) as s order by date");
